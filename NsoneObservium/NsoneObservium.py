...
class NsoneObservium:
    ...
    def __init__(self, observium_base_url, observium_user, observium_pass, additional_endpoints=[], ssl_verify=True,
                 timeout=10, observium_api_version=0):
    ...
    def _nsone_observium_generate_methods(self):
        ...
        for endpoint in self._api_endpoints:
            for action in endpoint['actions']:
                setattr(NsoneObservium, "%s_%s" % (action, endpoint['name']),
                        _nsone_observium_make_method(endpoint, action, maps))
...
def _nsone_observium_make_method(endpoint, action, maps):
    def _method(self, param_dict={}):
        ...
        try:
            ...
            # not using .get() because we need to delete the key/value pair if it exists anyway.
            # (We'll put it back later.)
            if id_key in param_dict:
                _id = param_dict[id_key]
                del param_dict[id_key]  # The *_id might not have been passed in, but we should delete it
                # if it was so that we avoid unintended query behaviour

            request_method = getattr(requests, maps['action_map'][action])
            # This and the next line could be condensed, but this is way more readable.

            response = request_method('%s/%s/%s' % (self._observium_api_base_url, endpoint['name'], _id), **{
            ...
            # Restore what we deleted a moment ago because the caller probably didn't expect us to play
            # with actual dict passed in as the method parameters.
            # This set after the previous delete should still be cheaper than doing something like a copy or
            # deep copy at the start of the method.
            if _id:
                param_dict[id_key] = _id

            if response.ok:  # Checking just ok here might not be enough.
                # We might need to specifically check for status_code == 200
                results = response.json()

                # Trying to make the resulting dict for single-item and multi-item search results uniform.
                # (A) If you search by a query term, you get something like "devices" with each id being a sub-key
                # to the device(s).
                # (B) If you search with a specific ID, you get something like "device"
                # with the details of the device in it.
                # The block below is trying to give the option of always finding results that look like (A) above.
                # It's just a tiny bit of memory to hold the extra reference to the same object, so very cheap
                # in the grand scheme of this module.
                ...
                # If there is no response.text, then this was probably an error before observium, such
                # as a connection error or a true 404.
                if not response.text:
                    response.raise_for_status()
                # Otherwise, we want to return the response that observium gave.
                else:
                    # The observium API responds with an http 404 error if you search for a device ID
                    # that doesn't exist.
